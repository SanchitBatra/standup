...
app.post("/comedianSignUp",function(req,res){
    ...
    Comedian.register(new Comedian({username : req.body.username , email : req.body.email , reviews : []}) , req.body.password , function(err,comedian){
        if(err){
...
app.get("/comedianForm",function(req,res)
{
    ...
    res.render("comedianDataForm.ejs" , {username : current_user});
});
...
app.post("/comedian/dataUploaded",function(req,res)
{
    ...
    console.log("Comedian Uploaded : "+comedian);
    var link = req.body.link;
    var title = req.body.title;
    var videoId = videoIdGenerator(link);
    var thumbNail = getThumbnail(videoId);
    var videoToDb = new pendingVideo({comedian : comedian , link: link,title : title,vid : videoId, thNail : thumbNail, reviewed : false});
...
app.get("/predict/:vidID", function(req,res)
{
    ...
    comedianLoggedIn = req.session.passport.user;
    ...
    pendingVideo.findByIdAndUpdate(vid , {reviewed : true} , {new : true} , function(err,foundVideo){
        if(err){
            console.log(err);
        }
        else
...
app.post("/postReaction", function (req, res,next) {
    ...
    var comedianLoggedInx = comedianLoggedIn;
    var data = req.body;
    console.log("Data from python : "+data);
    pendingVideo.find({comedian : comedianLoggedIn , reviewed : true , review : []}).then((video)=>{
        console.log("saaaaaalon"+video);
        video.review = [data.tsURL , data.cpURL];
        video.update({comedian : comedianLoggedIn , reviewed : true , review : []},video,function(err,raw){
            if(err){
                console.log(err);
            }
            else
